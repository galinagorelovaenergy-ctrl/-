import { Renderer, Program, Mesh, Color, Triangle } from "https://cdn.jsdelivr.net/npm/ogl@1.0.5/+esm";
...
function initPracticeGalaxy() {
  if (window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    return;
  }

  var wrap = document.getElementById("practice-galaxy-wrap");
  var host = document.getElementById("practice-galaxy-host");
  if (!wrap || !host) {
    return;
  }
  ...
  var targetMousePos = { x: 0.5, y: 0.5 };
  var smoothMousePos = { x: 0.5, y: 0.5 };
  var targetMouseActive = 0;
  var smoothMouseActive = 0;
  ...
  function update(t) {
    ...
    mu[0] = smoothMousePos.x;
    mu[1] = smoothMousePos.y;
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initPracticeGalaxy);
} else {
  initPracticeGalaxy();
}
